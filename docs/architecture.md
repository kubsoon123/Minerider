...
```text
crates/
  minerider-codegen/      build-time generator: minecraft-data → Rust (never a runtime dep)
      ↓ generates
...
packet dumpers) and fuzzed in isolation. The `minerider` crate depends on
it by path; shared dependency versions live in `[workspace.dependencies]`
in the root `Cargo.toml`.

All version-specific protocol data (packet ids, names, layouts, state and
direction mappings, the `ProtocolVersion` constants) is generated by
`minerider-codegen` from vendored minecraft-data into
`crates/minerider-protocol/src/generated/` — see
[codegen.md](codegen.md). Generated files are never edited by hand; a
drift test enforces that a fresh regeneration is byte-identical.
...
| `nbt` | Network NBT codec: full tag set, depth (64) and node (1M) caps |
| `holder` | `Holder<T>` / `HolderSet`: registry entry references vs inline values |
| `traits` | `Encode` / `Decode` + impls for primitives, `Box`, `Option`, `Vec`, fixed arrays; `ensure_consumed` |
| `generated` | minecraft-data output: `versions` (`ProtocolVersion`), `v1_21_4` (53 shared types + per-state packet structs/enums, integer-match dispatch). Never edited by hand — see [codegen.md](codegen.md) |
...
| `minecraft::handshake` | Handshake send helper over the generated `PacketSetProtocol` |
...
| generated decode keep alive (enum dispatch) | 14.2 ns | — |
| generated encode keep alive (id + payload) | 135.7 ns | — |
| generated decode client information | 62.3 ns | — |
| generated encode client information | 147.1 ns | — |
| generated packet-id dispatch (int match) | 1.1 ns | — |
| packet-name dispatch, string if-chain baseline | 2.0 ns | — |
| generated play dispatch (131 clientbound ids) | 1.5 ns | — |
...
- **Generated-codec tests**: golden byte-exact vectors for the login flow
  and configuration/play essentials, enum round-trips for every packet of
  the four small states plus a play selection (nested switches, option
  discriminants, NBT, metadata loops, holder sets), malformed-input
  rejection (truncation, overlong VarInts, oversized lengths, invalid
  mapper values, trailing bytes, NBT depth bomb), and a drift gate
  keeping committed generated files byte-identical to a fresh
  regeneration.
