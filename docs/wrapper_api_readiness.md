...
| `proxy` | Available | `ClientConfig::proxy: Option<Arc<Socks5ProxyConfig>>`, `network::socks5::Socks5ProxyConfig` | SOCKS5 transport is merged and in active use — the Lua wrapper's own trusted-proxy-profile model (`crate::lua::registry::ProxyProfiles`, see `docs/lua_wrapper.md#proxy-grouping-and-credentials`) is built directly on this field. A script never sees a `Socks5ProxyConfig`/credential directly; it only ever references a host-registered profile id, resolved to one of these by the runtime before a bot connects. |
